...
class SignInSubmitPasswordError(
    ...
    override  val errorCodes: List<Int>? = null,
    override var exception: Exception? = null
): SignInSubmitPasswordResult, BrowserRequiredError, Error(errorType = errorType, error = error, errorMessage= errorMessage, correlationId = correlationId, errorCodes = errorCodes, exception = exception) {
    fun isInvalidCredentials(): Boolean = this.errorType == SignInErrorTypes.INVALID_CREDENTIALS
}
...
open class SignInContinuationError(
    ...
    override val errorCodes: List<Int>? = null,
    override var exception: Exception? = null
): SignInResult, BrowserRequiredError, Error(errorType = null, error = error, errorMessage= errorMessage, correlationId = correlationId, errorCodes = errorCodes, exception = exception)
